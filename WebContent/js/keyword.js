$().ready(function(){
	...
	$("#keyword").blur(function(){
		...
		if(keyword.val() == '' || keyword.val().length <= 0){
			...
			//***
			$.post('valikey',{'keywords':keyword.val()},function(result){
				if('success' == result){
		...
		if(userid <= 0){
			...
			$.post('valikey',{'keywords':$("#keyword").val()},function(result){
				if('success' == result){
					...
					$.post('submitkeyword',{p:serviceType+"-"+years,'keywords':keyword,'customId':userid,'customName':userName},function(result){
						if("exception" == result){
...
function loadCustoms(){
	//**
	$.post("searchcustom",{'customName':$("#searchUerText").val()},function(result){
		//var json = result;
		var userList = "<ul>";
