$().ready(function(){
	...
	$("#addRoleSubmit").click(function(){
		...
		if(a_roleName == ''){
			...
			$.post(path+"/editrole/add",{'roleName':a_roleName,'isStart':a_isStart},function(result){
				if("success" == result){
					alert("添加成功！");
					window.location.href=path+"/rolelist";
	...
	$("#modifyRoleSubmit").click(function(){
		...
		if(m_roleName == ''){
			...
			$.post(path+"/editrole/modify",{'id':m_roleId,'roleName':m_roleName,'isStart':m_isStart},function(result){
				if("success" == result){
					alert("修改成功！");
					window.location.href=path+"/rolelist";
				}else if("repeat" == result){
	...
	$(".deleteRole").click(function(){
		...
		if(confirm("您确定要删除【"+d_roleName+"】这个角色吗？")){
			//delete
			$.post(path+"/delrole",{'id':d_roleId},function(result){
				if("success" == result){
					alert("删除成功！");
					//window.location.href==path+"/rolelist";
					window.location.href=path+"/rolelist";
