...
"newpwdtip1">新密码不少于6个字符</spa
	...
	$("#modifypwdbtn").click(function(){
		...
		if(oldpwd == ""){
			...
			$.post("modifypwd",{'userPassword':oldpwd,'userCode':newpwd,'userName':newpwd2,'isStart':1},function(result){
				if("success" .equals(result)){
					humane.success("恭喜您，密码修改成功。");
					$("#modifydiv").hide(500);
				}else if("2pwddif" .equals(result)){
					humane.error("您输入的新密码不一致，请重新输入。");
					$("#newpwd2").focus();
					$("#newpwdtip1").css("color","red");
				}else if("oldpwddif" .equals(result)){
